(function () {
  ...
  // Derive origin from this script's src so the iframe loads from the app host.
  ...
  function mount(config) {
    ...
    function setOpen(next) {
      open = next;
      if (open) {
        if (!iframe.src) iframe.src = scriptOrigin + "/embed/" + encodeURIComponent(botId);
  ...
  // Auto-init via data attribute: <script src=".../widget.js" data-bot-id="..."></script>
  try {
